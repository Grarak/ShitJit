...
fn cmp_and_branch(context: &mut Context, asm: &mut InstAssembler, code_cond: Code, addr: u64) {
    ...
    asm.uw_add_with_label(
        Inst::with2(Code::Mov_r64_imm64, Register::RSI, addr),
        &call_label,
    );
    ...
    asm.uw_add(Inst::with2(
        Code::Mov_r64_imm64,
        Register::RAX,
        utils::get_fn_addr!(Context::branch),
    ));
...
pub fn emit_beq(context: &mut Context, asm: &mut InstAssembler, operands: &[Operand]) -> bool {
...
pub fn emit_bne(context: &mut Context, asm: &mut InstAssembler, operands: &[Operand]) -> bool {
    assert_eq!(operands.len(), 1);
    let addr = get_label(&operands[0]);
